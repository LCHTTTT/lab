...
public class MySqlAccess {
    ...
    public void OpenSql() {
        try {
            string mySqlString = sjkcs.dbstr;
                /*"Database=" + databaseName + ";DataSource=" + host + ";User=" + userName + ";Password=" + password + ";port=" + port + ";charset=utf8;";*/
    ...
    public bool Insert(string tableName, string[] items, string[] value) {
        DataSet ds = QuerySet("Select id from students where number='" + value[0] + "'", Operate.Select);
        if (ds != null) {
            return false;
        }
        else {
