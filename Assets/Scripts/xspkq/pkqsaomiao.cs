...
public class pkqsaomiao : MonoBehaviour
{
                ...
                if (hit.Length > 0)
                {
                    for (int i = 0; i < hit.Length; i++)
                    {
                        if (hit[i].collider.name == "Sphere001")
                        {
                            ...
                            Debug.Log("酒精灯盖子");
                            gz.transform.localPosition = Vector3.MoveTowards(GameObject.Find("Sphere001").transform.localPosition, new Vector3(47.364f, 0.972f, 49.563f), Speed);
                            gzflag = 1;
                            //Debug.Log(gzflag);
                        }
